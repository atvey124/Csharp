...
//задач преобразовать двумерный массив символов в строку а затем,заменить заглавные буквы на строчные

Console.WriteLine("Введите размерность матрицы: ");
int size = int.Parse(Console.ReadLine()!);


char[,] RandomGenerator(int size)
{
...
string ConvertCharArrToStr(char[,] arr)
{
    string result = "";
    for(int i = 0;i < arr.GetLength(0);i++)
    {
...
char[,] RandomCharArr = RandomGenerator(size);

PrintFunction(RandomCharArr);

string result = ConvertCharArrToStr(RandomCharArr);

Console.WriteLine($"Получившееся слово: {result}");

UpperDown(result);

string new1 = UpperDown(result);

Console.WriteLine($"результат: {new1}");


// задача на нахождение является ли строка палиндромом
...
// программа которая отсортирует массив строк,по количеству элементов в строке
try{
...
// база данных позволяет хранить обрабатывать и быстро извлекать нужную информацию

// ирархическая структура не позволяет нам
...
представим себе,что наша база данных это телефонный справочник

                      ФИО                         НОМЕР ТЕЛЕФОНА          ДОП.НОМЕР         ДОП.НОМЕР       ДОП.НОМЕР

                    иванов иван иванович        12345678                46768789          5646565         5443577
                    петров петр петрович        12434566                64644646          -               -
                    андреев андрей андреевич    12455677                -                 -               -

из такой структуры вытекает множество проблем,например то,что у иванова номера 4 а андреева 1,тратиться очень много места,только изза одного иваново(представтье,что у иваново номеров будет тысяча),а у остальных 1 номер

кроме того,непонятно,что это за номера телефонов(рабочий,второй,старый),мы не можем сразу назвать ячейки: рабочий,второй,старый.Поскольку точно не знаем какой куда отнесем,а так же рабочих номеров может быть два,а место в ячейке одно

именно поэтому мы можем организовать нашу структуру подругому

                            ФИО                  НОМЕР ТЕЛЕФОНА          КОМЕНТАРИЙ

                    иванов иван иванович           12345678             основной номер
                    иванов иван иванович           54565635             рабочий номер
                    иванов иван иванович           54546665             старый номер
                    иванов иван иванович           54536355             второй номер

                    петров петр петрович           12434566             основной
                    петров петр петрович           43665675             второй

                    андреев андрей андреевич       12455677             основной

таким образом мы можем не выделять в нашей базе данных 4 места под номера телефона,только из-за одного иванова,а так же видим конкретное описание,коментарий

однако даже у такой структуры есть большие проблемы,представим к примеру,что кроме номеров,нам нужно хранить еще и факт.адрес статус и дату рождения

                            ФИО                  НОМЕР ТЕЛЕФОНА          КОМЕНТАРИЙ                 Д/Р                 ФАКТ.АДРЕС              СТАТУС

                    иванов иван иванович           12345678             основной номер             01.02.2000             Москва                женат
                    иванов иван иванович           54565635             рабочий номер              01.02.2000             Москва                женат
                    иванов иван иванович           54546665             старый номер               01.02.2000             Москва                женат
                    иванов иван иванович           54536355             второй номер               01.02.2000             Москва                женат

                    петров петр петрович           12434566             основной                   02.03.1999             Казань                женат
                    петров петр петрович           43665675             второй                     02.03.1999             Казань                женат

                    андреев андрей андреевич       12455677             основной                   05.05.1987             Казань                женат


тогда если допустим у человека изменился факт.адрес то нам надо пройти каждую строчку и поменять там адрес(представтье,что номеров у иванова может быть 1000,строчек с его фамилией тоже будет тысяча соответственно),однако факт.адрес статус и д/р везде будет одинаково,ведь указывает на одного человека,однако нам все равно нужно будет пройти эту тысячу строк и изменить данные

однако этому снова есть решение

разобьем нашу таблицу на две состовляющие


                                            ЧЕЙ ТЕЛЕФОН                                                                                    СТАТУС   
                            ФИО                  НОМЕР ТЕЛЕФОНА          КОМЕНТАРИЙ                                ФИО              Д/Р                 ФАКТ.АДРЕС              СТАТУС

                    иванов иван иванович           12345678              основной номер                иванов иван иванович       01.02.2000             Москва                женат
                    иванов иван иванович           54565635              рабочий номер                 петров петр петрович       02.03.1999             Казань                женат
                    иванов иван иванович           54546665              старый номер                  андреев андрей андреевич   05.05.1987             Казань                женат
                    иванов иван иванович           54536355              второй номер                              
...
таким образом нам не придется заполнять тысячу раз статус иванова и тратить на это очень много памяти,мы просто возьмем информацию из второй таблицы,указав фамилию                 

однако и тут кроется очевидная проблема,у нас нет какого то индефикатора по которому мы можем связывать два списка,пока что они связаны только фамилией,слева мы видим номер к примеру иванова,а справа его статус
может быть такое,что у людей будут совпадать фио полностью,совпадать адрес проживания и статус и тогда мы никак не отличим одного иванова от другого иванова,тоесть надо добавить индефикатор,по которому мы можем связывать два списка,тоесть к примеру запросить с первой таблицы уникальный индефикатор и оно нам выдаст номер и запросить со второй таблицы этот индефикатор и оно нам выдаст статус
другими словами,для того чтобы связать две и более таблицы,нам нужен уникальный индефикатор,ведь в двух таблицах могут быть полные тески
и этот индефикатор называется первичный ключ


вот так будет выглядеть наша база данных уже с ключом

                                           ЧЕЙ ТЕЛЕФОН                                                                                    СТАТУС   
           key                ФИО                  НОМЕР ТЕЛЕФОНА          КОМЕНТАРИЙ                    key                    ФИО               Д/Р                 ФАКТ.АДРЕС              СТАТУС

            1          иванов иван иванович           12345678              основной номер               1       иванов иван иванович       01.02.2000             Москва                женат
            1          иванов иван иванович           54565635              рабочий номер                3       петров петр петрович       02.03.1999             Казань                женат
            1          иванов иван иванович           54546665              старый номер                 4       андреев андрей андреевич   05.05.1987             Казань                женат
            1          иванов иван иванович           54536355              второй номер                 2       иванов иван иванович       06.07.1990             Санкт-Петербург       холост           
            
            2          иванов иван иванович           54565657              основной телефон

            3          петров петр петрович           12434566              основной                                                
            3          петров петр петрович           43665675              второй                                                  

            4          андреев андрей андреевич       12455677              основной 

и так мы смотрим на таблицу слева,нас интересует пользователь с ключем '1' мы обращаемся к таблице справа и запрашиваем оттуда информацию о нем,по тому же ключу и так далее
мы видим,что ключ слева один и тот же для одного иванова,а для другого он уже другой,таким образом мы больше не привязаны к фамилии и она нас вообще не интересует
в левой таблице ключ у иванова повторяется 4 раза,так происходит потому что он ссылается на одного и того же человека,такой ключ называют вторичным


попробуем добавить в нашу структуру еще одну таблицу с адресом и будем так же связывать ее с остальными по ключу

                                           ЧЕЙ ТЕЛЕФОН                                                                                    СТАТУС                                                                            АДРЕС   
           key                ФИО                  НОМЕР ТЕЛЕФОНА          КОМЕНТАРИЙ                    key                    ФИО               Д/Р                СТАТУС                                          key           адрес прожив.
                                                                                                                                                                                                                            
            1          иванов иван иванович           12345678              основной номер               1       иванов иван иванович       01.02.2000               женат                                            1                москва
            1          иванов иван иванович           54565635              рабочий номер                3       петров петр петрович       02.03.1999               женат                                            3                казань  
            1          иванов иван иванович           54546665              старый номер                 4       андреев андрей андреевич   05.05.1987               женат                                            4                казань
            1          иванов иван иванович           54536355              второй номер                 2       иванов иван иванович       06.07.1990               холост                                           2                санкт-петербург
...
таким образом нам стало еще легче ориентироваться,поскольку данные во всех этих таблицах связанны ключем,не сами таблицы а именно данные в этих таблицах

ключем может быть что угодно,к примеру на нашем сайте регестрируется новый пользователь и мы даем ему этот ключ(уникальный индефикатор),а далее заносим его в нашу базу данных в эти таблицы,ключ уникален и будет отличать одного иванова от другого
...
ТАКУЮ СТРУКТУРУ НАЗЫВАЮТ РЕЛЯЦИОННОЙ - это база данных в которой данные распределены по отдельным связанным между собой таблицам(засчет ключа)



// псевдокод SQL
/*
SELECT*FROM общий список - выведется вся информация из таблицы 'общий список', '*' - как раз и означает что выбирается вся информация

SELECT ФИО,ТЕЛ,АДРЕС - ФИО,ТЕЛ,АДРЕС - являются атребутами,подругому просто столбцами в нашей таблице

FROM общий список - тут происходит выборочный вывод информации,тоесть мы хотим вывести ФИО,ТЕЛ,АДРЕС из нашей таблицы

WHERE статус = "холост" - далее добавляется условие,оно обозначается 'WHERE',тоесть наша информация выберется только оттуда где условие соблюдено


и так еще раз 'SELECT' - нужен когда мы хотим выбрать какую-то информацию,если добавить * 'SELECT*' - то выберется вся информация
'FROM' - обозначает из какой таблицы мы хотим получить данные(выбрать)
'WHERE' - означает условие

в главную таблицу обычно заносится информация которая не может иметь несколько разных вариантов к примеру СТАТУС,Д/Р,ФИО - в отличии от адреса у нас не может быть два ФИО или два Д/Р

Нам нет смысла разбивать эту информацию по разным таблицам и там связывать их ключем,мы можем занести эту информацию в одну таблицу,которая будет иметь значения у которых 1 вариант

обычно в такой таблице мы создаем ключ и называем его id человека - такой клчюч будет называться первичным,назовем эту таблицу ЧЕЛОВЕК

далее можем создать таблицу номеров и связать эти две таблицы по ключу 'id чей телефон',посути этот айди это 'id человека' - в таком случае это будет внешний ключ,поскольку мы связываем им две таблицы,назовем эту таблицу НОМЕР

можем так же создать еще и таблицу адресов и так же связать ее внешним ключем 'id человека',назовем эту таблицу АДРЕС


и так мы умеем работать только с одной таблицей,но как нам объеденить две или даже три таблицы в результат
для этого существует много вариантов,к примеру inner join - что означает,что результатом будут данные которые относятся как к одной таблице так и к другой.Оно берет две таблицы,левую и правую и выдает в результат,то что относится как к левой таблице так и к правой
к примеру и в левой и в правой таблице у нас будет связь по ключу 'id человека',результатом будет две таблицы и этот ключ


LEFT JOIN - означает,что из двух выбранных таблиц преобладать будет левая(преобладать в данном случае значит,что все данные из этой таблицы попадут в итоговую таблицу - в наш результат),но так же будет выведена информация из пересечения таблиц.далее для простаты левая таблица будет названа - 1,правая - 2
RIGHT JOIN - означает тоже самое что и LEFT JOIN,но преобладает уже таблица номер 2
FULL JOIN - означает,что в результат будет выбрана информация из таблицы 1 и таблицы 2,а так же их пересечение


INNER JOIN ЧЕЛОВЕК,НОМЕР - ЧЕЛОВЕК,НОМЕР - то что мы хотим найти(внести в новую таблицу)
ON id человека = id чей телефон - ON это условие,тоесть если ключ из первой таблицы 'id человека' равен ключу из второй 'id чей телефон',то тогда эта информация будет выведена в результат
ПРИМЕР:
        ЧЕЛОВЕК                                                                                     НОМЕР
id человека       ФИО            Д/Р                                                       id чей телефон       ТЕЛЕФОН 
    1         иванов и.и        01.02.2006                                                       1               6356
    2         иванов и.и        01.02.2006                                                       1               6354
    3         иванов и.и        01.02.2006                                                       1               6358

РЕЗУЛЬТАТОМ БУДЕТ новая таблица

id человека       ФИО            Д/Р         id чей телефон       ТЕЛЕФОН 
    1         иванов и.и        01.02.2006          1               6356
    1         иванов и.и        01.02.2006          1               6354
    1         иванов и.и        01.02.2006          1               6358
люди под индексом id человека 2,3 не имеют номера,id 1 встречается и в первой и во второй таблицы,соотвественно и в новую таблицу попадет вся информация о человеке с индексом 1,id 2,3 встречается только в первой таблице,во второй не встречается,соотвественно в новую таблицу индекс 2,3 не попадет
назовем новую таблицу - новая таблица
и после того как мы объединили эти таблицы мы можем извлечь нужную информацию

ПРИМЕР:
SELECT ФИО ТЕЛЕФОН
FROM новая таблица
INNER JOIN ЧЕЛОВЕК,НОМЕР
ON id человека = id чей телефон

в этом случае компьютер будет судить о этих двух таблицах как о одной,поскольку мы выполнили inner join - у него обязательно должно быть условие 'ON' - чтобы найти то,что пересекается 

добавим еще одну таблицу

        ЧЕЛОВЕК                                                                                     НОМЕР                                                           АДРЕС   
id человека       ФИО            Д/Р                                                       id чей телефон       ТЕЛЕФОН                            id чей адрес           адрес         комментарий
    1         иванов и.и        01.02.2006                                                       1               6356                                   1               москва              место прописки
    2         иванов и.и        01.02.2006                                                       1               6354                                   2               москва              место прописки
    3         иванов и.и        01.02.2006                                                       1               6358                                   3               москва              место прописки
    4         петров п.п.       01.03.2000                                                       4               5465                                   4               москва              место прописки
                                                                                                                                                        4               питер               вторая прописка


INNER JOIN ЧЕЛОВЕК,АДРЕС
ON id человека = id чей адрес


                                                НОВАЯ ТАБЛИЦА,как результат выполнения INNER JOIN
id человека       ФИО            Д/Р                    АДРЕС             КОММЕНТАРИЙ                   id чей адрес
    1         иванов и.и        01.02.2006             москва               место прописки                   1
    2         иванов и.и        01.02.2006             москва               место прописки                   2
    3         иванов и.и        01.02.2006             москва               место прописки                   3
    4         петров п.п.       01.03.2000             москва               место прописки                   4
    4         петров п.п.       01.03.2000             питер                вторая прописка                  4


далее можем взаимодействовать с новой таблицей и выбрать оттуда какие нибудь аргументы(столбцы)
SELECT ФИО,АДРЕС,КОММЕНТАРИЙ
FROM НОВАЯ ТАБЛИЦА

ПОДИТОЖИМ:
INNER JOIN - включает в себя объединение множеств,если ключ id человека к примеру 1 = соотвествует ключу id адрес во второй таблице - то он попадает в новую таблицу,если какой то из ключей не соответствует(тоесть к нему нет пары во второй таблицы),к примеру id человека 1,для него не нашлось ключа id чей адрес 1,то тогда мы игнорируем первую и вторую таблицу,в новую таблицу не попадут данные для ключа id человека и id чей адрес,не из первой таблицы не из второй
поскольку INNER JOIN - вносит в новую таблицу только те данные которые встречаются и там и там


теперь поговорим о LEFT JOIN - он вносит в новую таблицу все данные из левой таблицы,а так же пересечение со второй,к примеру
есть две таблицы

        ЧЕЛОВЕК                                                                                     НОМЕР
id человека       ФИО            Д/Р                                                       id чей телефон       ТЕЛЕФОН 
    1         иванов и.и        01.02.2006                                                       1               6356
    2         иванов и.и        01.02.2006                                                       2               6354
    3         иванов и.и        01.02.2006                                                       3               6358
                                                                                                 4               5476

мы делаем LEFT JOIN ЧЕЛОВЕК,НОМЕР - ЧЕЛОВЕК - является левой таблицей,НОМЕР - является правой
ON id человека = id чей телефон
и получаем новую таблицу,куда запишем всю таблицу ЧЕЛОВЕК и пересечение с таблицей НОМЕР
...
id чей телефон = id человека,id человека 4 не существует в 4 таблице,соотвественно пересечения двух множеств нет для индекса 4,соотвественно в новую таблицу он не попадает 
однако если мы поменяем местами таблицы при вызове LEFT JOIN - то новая таблица будет кардинально другой

LEFT JOIN НОМЕР,ЧЕЛОВЕК
ON id человека = id чей телефон
...
однако результат будет другим - по правилу,LEFT JOIN - мы должны занести в новую таблицу всю информацию из левой таблицы и пересечение с правой,левая таблица - НОМЕР,и мы видим,что для индекса 4 нет пересечения в правой таблицы - ЧЕЛОВЕК,произойдет вот что
...
2              иванов и.и     01.02.2006            2               6354
3              иванов и.и     01.02.2006            3               6358
null           null           null                  4               5476

тоесть мы оставляем эти ячейки с пустым значением в C# - это 'null'


в RIGHT JOIN - принцип тот же,только преобладающей таблицей будет правая
...
id человека       ФИО            Д/Р                                                       id чей телефон       ТЕЛЕФОН 
    ...
    2         иванов и.и        01.02.2006                                                       2               6354
    3         иванов и.и        01.02.2006                                                       3               6358
    4         петров п.п.       01.02.2007                                                       4               5476
    5         петров п.п.       01.02.2009                                                       7               6565
    6         матвей в.в.       02.06.2008                                                       6               5656
    7         алексей и.т.      02.06.2010                                                       2               6565

RIGHT JOIN  НОМЕР,ЧЕЛОВЕК
ON id человека = id чей телефон
...
1         иванов и.и        01.02.2006             1                6356
2         иванов и.и        01.02.2006             2                6354
2         иванов и.и        01.02.2006             2                6565
3         иванов и.и        01.02.2006             3                6358
4         петров п.п.       01.02.2007             4                5476
5         петров п.п.       01.02.2009             null             null
6         матвей в.в.       02.06.2008             6                5656
7         алексей и.т.      02.06.2010             7                6565

все работает так же как и с LEFT JOIN


и последний способ объединения двух таблиц 'FULL JOIN' - объединяет в себе и LEFT JOIN и RIGHT JOIN
...
id человека       ФИО            Д/Р                                                       id чей телефон       ТЕЛЕФОН 
    ...
    6         матвей в.в.       02.06.2008                                                       6               5656
    7         алексей и.т.      02.06.2010                                                       2               6565
    8         алексей и.т.      02.06.2011                                                       10              6565
    9         алексей и.т.      02.06.2014


FULL JOIN ЧЕЛОВЕК,НОМЕР - порядок таблиц в вызове 'FULL JOIN' -  не имеет значения,ведь в новую таблицу должнап попасть и левая и правая таблица соотвественно
ON id человека = id чей телефон

                                                                НОВАЯ ТАБЛИЦА,как результат выполнения 'FULL JOIN'
...
6         матвей в.в.       02.06.2008          6               5656
7         алексей и.т.      02.06.2010          7               6565
8         алексей и.т.      02.06.2011          null            null
9         алексей и.т.      02.06.2014          null            null
null      null              null                10              6565 

принцип тот же,заносим все элемеенты двух таблиц в новую - по правилу,однако те которые не пересекаются(не имеют одинаковых ключей) - получают пустое значение(null)
...
/*
генетический алгоритм - это способ и оптимизация решений,прообразом которого стал - естественный отбор в реальной жизни


1.он имеет n количество хромосом(возможных вариантов решения той или инное задачи),это первый пункт - инициализация хромосом

2.Сравнение насколько мы близки к лучшему решению задачи

3.если не близки
 1.выбираем лучшие хромосомы(путь решения задачи) из текущего поколения
 2.далее нужно провести некоторые операции(скрещивание,мутация)
    мутация - оператор который с некоторой вероятностью меняет один или несколько генов(элементов в наших хромосомах(путях решения) в хромосомах в случайном порядке,простыми словами),такой оператор позволяет оброзовать новую популяцию и так же запустить в ней генетический алгоритм
 3.создаем новую популяцию,тем самым сокращая хромосомы и отсеивая не нужные и идем так по кругу
4.если мы стали близки к лучшему решению задачи - генетический алгоритм заканчивается
