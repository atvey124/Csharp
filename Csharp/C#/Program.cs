...
// факты о C#
// C# - является строго типизируемым яп
// C# - используем управляемую среду выполнения(тоесть выделять оперативную память под к примеру массив либо объект класса) - не нужно
...
// сокращенный оператор 'И' - '&&'
// bool HasNoColling,HighTemp = true;
//ПРИМЕР: if(HasNoColling && HighTemp) - в таком случаем логический оператор - '&&' - выдаст true - ведь два условия соблюдены
//в случае если хоть одно из условий не соблюдается - логический оператор '&&' - выдаст false
//тоесть если мы поставим к примеру сто таких условий и объеденим их с помощью оператора - '&&',в таком с улчае если давже 99 условий будут верны а последнее нет,во всем условие 'if' - будет выдано false


// сокращенный оператор 'или' - '||'
// bool HasNoColling = false;
// bool HighTemp = true;
//if(HasNoColling || HighTemp) - в таком случае оператор '||' - выдаст true,ведь он проверяет соблюдается ли хоть одно из условий
//не смотря на то что первое условие 'HasNoColling' - не соблюдается,второе условие 'HighTemp' - соблюдается,таким образом мы проходим внутрь блока 'if'


//ПОДЫТОЖИМ

// логический оператор '&&' - выдаст true,только в том случае если все условия соблюдены(мы как бы говорим программе,что и то и то должно быть соблюдено)
// логический оператор '||' - выдаст true,даже в том случае если одно условие соблюдается,даже если из 100 условий соблюдается 1,оператор '||' - выдаст true(мы как бы говорим программе - либо то либо то)
// однако эти операторы (|| &&) - являются сокращенной формой операторов - & |
// bool Test = false;
// bool Test2 = true;
// bool Test3 = true;
//единственное их отличие в том,что при использовании оператора && - если мы сразу видим что первое условие не соблюдено,к примеру будет так(if Test && Test2 && Test3) - тоесть тут мы сразу видим что первое условие уже не соблюдено,и оператор && - это тожен заметит и не будет проверять все оставшиеся условия,он сразу выдаст false
// к примеру представим,что таких условий будят тысяча и на середине мы увидим,что одно из условий не соблюдено,мы не будем проверять оставшиеся операторы,а сразу закончим
// таким образом работает оператор '&&' - он не проверяет оставшиеся условия,после того как нашел не соблюдение - он сразу выдаст false
// а вот оператор & - даже если увидит,что условие не соблюдено в начале,он все равно проверит все условия и только потом выдаст либо true либо false
// так же и с оператором || - если он увидит,что условие соблюдается он сразу выдаст true - и не будет просматривать остальные условия
//к примеру из ста условий с использованием оператора ||,первое же условие соблюдается - оператор сразу же выдает true и проверка останавливается
// а вот оператор | просмотрит все условия,даже если первое условие выполняется
//ПОДЫТОЖИМ: 

// || - сокращенный оператор 'или' - когда одно из условий соблюдается,таких условий может быть тысяча и если одно из них выполняется а 999 нет,то оператор '||' - все равно выдаст true и зайдет в блок if

// && - сокращенный оператор 'и' - из тысячи должны соблюдаться все условия,даже если 999 соблюдается,а 1 - нет,&& - выдаст false.Если все 1000 соблюдается && - выдаст true, и зайдет в блок if
// & - полный оператор 'и' - тоже самое что и сокращенный,только остановит проверку после просмотра всех условий,а не когда увидит,что одно из них не соблюдается
// практического приминения не имее,ведь нам нет смысла проверять 999 оставшихся условий если 1 условие уже не соблюдено
// | - полный оператор 'или' - тоже самое что и полный оператор 'и' - если из тысячи первое условие соблюдено,он не остановит проверку сразу - а остановит только тогда когда проверит все условия
// так же нет практического приминения,тормозит код


                                                                                        // операторы условия 
//ТЕРНАРНЫЙ ОПЕРАТОР
/*
    давайте сначала определимся с тем что такое бинарный и унарный оператор
    унарным называют тот оператор у которого один оперант
ПРИМЕР унарный оператор:
    int a = 1;
    a++; - данный оператор называется унарным поскольку мы производим операции только с одним операндом
ПРИМЕР бинарный оператор:
    int a = 1,b = 2;
    int c = a + b; - + в данном случае является бинарным оператором,поскольку производит вычисления с двумя операндами(a + b)
    Console.WriteLine(a + b); - в данном случае + тоже является бинарным оператором
    int c = 1,d = 2,g = 3,e = 4;
    int plus = c + d + g + e; - в данном случае любой из '+' - тоже является бинарным,поскольку данный пример будет считан компьюетром,как: (c + d) = какое-то число,далее (какое-то число + g) = какое-то число1,далее (какое-то число1 + e) = plus,тоесть любой математический пример является бинарным
    а вот дикремент и инкримент является унарным,так же унарным является и Console.WriteLine(plus) и подобные вещи которые работают с одним операндом

ТРЕНАРНЫЙ ОПЕРАТОР ТЕОРИЯ:
    к примеру мы хотим узнать совпадает ли пароль,что ввел пользователь с паролем,что хранится в нашей бд
    string true_password = "qwerty"; 
    string input_password = Console.ReadLine();
    bool is_equale;

    ранее для сравнения этих двух переменных мы могли воспользоваться конструкцией if else
    либо же передать в переменную 'is_equale' - сравнение наших переменных,таким образом is_equale = true_password == input_password; - таким образом наша переменная будет хранить в себе либо true либо false

    однако теперь мы можем воспользоваться тернарным оператором

    is_equale = true_password == input_password; - выглядит как и то что мы делали до этого,однако можем добавить тернарный оператор

    is_equale = true_password == input_password ? true : false; - знак вопроса после нашей логики - означает,что мы как бы хотим узнать результат выполнения операции,далее после знака '?' идет,то что будет присвоено в переменную 'is_equale' - в том случае,если наша логика 'true_password == input_password' возвращает true
    далее идет знак ':' - который указывает,что будет занесено в переменную если логика 'true_password == input_password' вернет false, в данном случае в переменную is_equale запишется значение false
ПОДЫТОЖИМ:
    как мы выяснили тернарный оператор схож с конструкцией if else,а точнее является ее аналогом,тоесть когда мы пишем любую логику(которая возвращает либо true либо false) - мы можем использовать тернарный оператор,как в примере выше
    и описать действия над переменной в зависимости от того,что вернет наша логика
    единственное отличие тут,что мы можем взаимодействовать таким образом только над переменными,либо к примеру return в методах
    это позволяет сократить код и избежать излишних конструкций
ТЕРНАРНЫЙ ОПЕРАТОР ПРИМЕР:
    int input_num = int.Parse(Console.ReadLine())
    bool num_is_odd = input_num % 2 == 0 ? true : false; - в данном случае мы хотим выяснить является ли введенное пользователем число четным
    для этого мы воспользуемся тернарным оператором и запишем в переменную 'num_is_odd' - результат тернарного оператора
    если наша логика вернет true то выполнится действие между ? :,если false то выполнится действие после :

    ДАННЫЙ КОД ЯВЛЯЕТСЯ ПРОСТЫМ ПРИМЕРОМ И ЕГО МОЖНО ЗАПИСАТЬ ИНАЧЕ И КОМПАКТНЕЕ,ОДНАКО ДЛЯ ПОНИМАНИЯ Я ИСПОЛЬЗУЮ ЭТОТ ПРИМЕР


    к примеру мы хотим узнать является ли число что ввел пользователем отрицательным,если да то выведем 0,если нет то само число

    int input_num = int.Parse(Console.ReadLine());
    int result = input_num < 0 ? 0 : input_num; // проверяем на логику,если возвращает true - присваиваем переменной result значение 0,есле вернет false присваиваем переменной result значение переменной input_num
    делается это напрямую,не нужно указывать в блоке ? result = 0 : result = input_num
    ДЕЛАЕТСЯ ЭТО НАПРЯМУЮ,КАК В ПРИМЕРЕ ВЫШЕ

    к примеру у нас будет функция,которая будет принимать массив и нам нужно посчитать количество четных и не четных элементов

    void GetOdd(int[] basic_arr)
    {
        int odd = 0; // переменная для не четных
        int even = 0; // переменная для четных
        int number_is_odd; // переменная для вызова тернарного оператора,если в блоке тернаного оператора мы будем производить операции над числами - то мы указываем тип переменной int,если же нам нужно получить булевое значение,то указывааем тип данных bool
        // по аналогии и с другими типами данных
        for(int i = 0;i < basic_arr.Length;i++) number_is_odd = basic_arr[i] % 2 == 0 ? even++ : odd++; // вызываем логику внутри переменной и в зависимости от результат пишем тренарный оператор(тоже самое что и if else)
        System.Console.WriteLine(odd); // 3
        System.Console.WriteLine(even); // 3
    }

    GetOdd(new int[6]{1,2,3,4,5,6});

    к примеру мы хотим узнать количество букв в верхнем регистре 
...
ПОДЫТОЖИМ:
    тернарным оператором является тот оператор у которого три операнда
    bool num_is_odd = input_num % 2 == 0 ? true : false; - в данном примере первый оператор переменная 'num_is_odd',второй то что выполняется в блоке ? :,третий то что выполняется в блоке :
    важным является,что этот оператор работает только с логикой(тип данных bool) как и конструкция if else,однако операции которые он проводит в зависимости от результата логики - могут иметь другой тип данных
    такой тип данных и должна иметь переменная внутри которой мы проводим тренарный оператор
...
int cash,food,FoodPrice = 10; // обхявляем переменные для количества денег пользователя,количества еды,что он хочет купить и цене за одну штуку еды
bool MoneyCount; // объявляем переменную,с типом данных bool,чтобы узнать хватает ли у пользователя денег,чтобы купить такое количество еды
Console.WriteLine($"Сегодня в магазине акция,вся еда по {FoodPrice} цене");

Console.Write("Введите сколько денег у вас есть: ");
cash = int.Parse(Console.ReadLine()!);
Console.WriteLine("Введите сколько еды вы хотите купить");
food = int.Parse(Console.ReadLine()!);

MoneyCount = cash >= food * FoodPrice; // в белвую переменную попадет либо true - если количество денег пользователя больше количества денег за еду,либо false - в противном случае
cash -= Convert.ToInt32(MoneyCount) * (food * FoodPrice); // далее переводим булевую переменную в int(true - 1,false - 0),таким образом получаем либо 0 либо 1 и умножаем на цену за еду,если денег не достаточно,баланс не снимется и еда у пользователя не появится

Console.WriteLine($"у вас осталось {cash} денег,у вас {food} еды ");




int Basic_damage = 10,Health,RoundCount,Armor,PercentConvertor = 100,RoundComplete = 0; // инициализируем переменные,что нам потребуются
bool IsDead = true; // переменная для проверки проигриша

Console.WriteLine($"тебе предстоит сражение с боссом,выбери свои очки здоровья: ");
Health = int.Parse(Console.ReadLine()!);

Console.Write("Введите сколько раундов ты хочешь продержаться: ");
RoundCount = int.Parse(Console.ReadLine()!);

Console.Write("Введите количество защиты(в процентах): ");
Armor = int.Parse(Console.ReadLine()!);

bool DefeatOrNot(int Basic_damage,int Health,int RoundCount,int Armor,int PercentConvertor,int RoundComplete,bool IsDead){

    while(IsDead){ // пока булевая переменная не равна ложному значению
        IsDead = Health >= Basic_damage * Armor / PercentConvertor; // передаем в булевую переменную сравнение текущих очков здоровья - урон полученный от босса,если они меньше засчитываем проигришь
        Health -= Basic_damage * Armor / PercentConvertor; // от очков жизни отнимаем урон умноженный на защиту(получаем 500,в случае если 50 очков защиты) и делим на 100,таким образом получаем процент
        RoundComplete++;
        if(RoundComplete > RoundCount){ 
            return IsDead;
        }
...
bool ResultBool = DefeatOrNot(Basic_damage,Health,RoundCount,Armor,PercentConvertor,RoundComplete,IsDead); // таким образом мы получаем результат поединка и можем использовать его в дальнейшем
...
void PrintLast_first(int[] arr) // выводит пары чисел,в каждой паре содержится элемент с начала массива с элементом с конца
{
    int length_array = arr.Length / 2; // если в массиве нечетное количество элементов,то тогда элемент который в середине будет утерян,мы же хотим вывести для него пару(парой для этого элемента будет этот же элемент)
    if(arr.Length % 2 != 0) length_array += 1;
    for(int i = 0,j = arr.Length - 1;i < length_array;i++,j--){ // в цикле можно задавать несколько переменных,таким образом создаем одну переменную для отсчета с начала,вторую для отсчета с конца(поскольку мы выводим пары элементов - условием остановки цикла,будет длина деленная на два),так же указываем инкримент для переменной,что выводит число с начала и дикремент для переменной,что выводит элемент с конца
        System.Console.WriteLine($"{arr[i]}:{arr[j]}"); // выводим пару
    }
}

PrintLast_first(new int[9]{1,2,3,4,5,6,7,8,9});
...
int i = 0;
i++; // данная операция называется постфиксной
++i; // данная операция называется префиксной,тоесть имеет больший приоритет
Console.WriteLine(i++); // к примеру если мы захотим вывести i++,то у нас сначала выведется i,а уже потом она увеличится на один
Console.WriteLine(++i); // если же мы захотим вывести i используя префиксный операторо,мы сначала увеличим i на 1,а уже потом выведем
// подитожим: данные операторы имеют разную степень приоритета,постфиксная имеет высший приоритет,тоесть сначала выполняется она,а уже потом действие,с постфиксным все наоборот
// так же такие операции называют инкриментом и дикрементом i++ - является инкреиментом,ну и наоброт ++i - тоже инкримент,тоесть увеличение числа на один
// ну и дикремент это уменьшение числа на один: i-- --i
// что важно запомнить,они имеют высший приоритет,тоесть выполняются в первую очередь,допустим в каком нибудь примере,их приоритет даже выше умножения и деления

int A = 0;
Console.WriteLine(++A + 1 + 3 + "2" + A++ + ++A); // простой пример с использованием инкримента с постфиксной и префиксной формой,в результате получим 5213 - поскольку в примере стоит строка,а это значит что весь пример тоже будет иметь тип данных строка,до строки у нас получилось число 5,сама строка является '2',тоесть после конкатенации мы получим 52,а уже после мы будем не складывать числа а конкатенировать с этой строкой '52'
...
// объявление массива может выглядеть и таким образом int[] MyArray;
// далее мы просто зададим ему размер MyArray = new int[10]; -  тоесть если мы заранее не знаем сколько элементо он будет в себя включать но точно знаем,что будем его использовать
// можно объявить его указав тип данных и имя,а уже после присвоить ему размер(к примеру после того как посчитаем сколько данных нам нужно хранить)
// оператор 'new' - означает что в управляемой куче м выделяем память под массив
// в самой же переменной MyArray - будет храниться ссылка на этот объект памяти в котором хранится сам массив(поскольку массивы являются ссылочными типами данных)
// при таком объявлении массива 'int[] MyArray' - по стандарту он будет иметь значение null - это стандартное значение для ссылочных объектов,null - означает,что ссылка переменной ссылочного типа не ссылается ни на какую область памяти(поскольку мы еще не выделили область в памяти под элементы этого массива)
// после того как выделим MyArray будет ссылаться на эту область в памяти и будет иметь значение из этой области памяти
// так же индексы в массиве,это посути номера внутри этой области памяти,которую мы выделили
...
namespace BasicOOP
{
    ...
    class Point // в данном классе я хочу описать расположение точки на координатной плоскости,так же каждый класс можно представить в виде чертежа дома,из которого можно построить дом.А вот экземпляр класса это уже построенный дом по чертежу дома
    {
        public int x; //для этого нам понадобиться переменная которая будет хранить ее расположение по оси 'x',переменные в ООП называют - полем,тоесть x - это поле класса Point
        public int y; // так же понадобиться поле которое будет хранить расположение точки по оси 'y'

        public string color = ""; // добавим еще одно описание объекта - цвет точки

        public Color color1; // теперь же в шаблоне Point - мы создали объект шаблоона 'Color' - который хранит все допустимые цвета

        // 'public' - нужно для того чтобы обеспечить доступ к данным класса,всем понятно,что такой код не является надежным
        // и вот чтобы сделать его более надежным,нужно осуществлять доступ к данным класса с помощью свойств
    } 
    // подытожим,class 'Point' - является чертежом дома,тоесть описывает,характеристики обЪекта - в нашем случае точки

    // а в классе 'Program' - мы создаем экземпляр класса - тоесть саму точку,далее наделяем характеристики нашими данными

    // так же class - является ссылочным типом данных


    class Program // так же класс можно представить как свой собственный тип данных,тоесть как мы указываем int x,так же можно указать и Point x
    {
        static void Main(string[] args){ // если мы называем функцию внутри класса 'Main' - она автоматически является точкой входа,тоесть вызывать ее не нужно

            Point point1 = new Point(); // тут мы создаем экземпляр класса, и поскольку класс является ссылочным типом данных мы должны указать new Point();,тоесть мы как бы размещяем его тут
            
            point1.x = 4; // далее мы для созданой точки используем поле из класса 'Point' - которое хранит все характеристики нашей точки и наделяем их значением 
            point1.y = 2;
            Console.WriteLine($"Координата по оси x: {point1.x} | Координата по оси y: {point1.y} "); // здесь мы увидим характеристики этой чоки

            Point point2 = new Point(); // создадим еще одну точку и будем использовать тот же шаблон для нее

            point2.x = 3;
            point2.y = 4;
            Console.WriteLine($"точка {point2} Координата по оси x: {point2.x} | Координата по оси y: {point2.y} "); // если же в выводе мы укажем '{point2}' - мы увидим пространство имен где эта точка лежит,а так же ее клас,поскольку каждый объект создается по шаблону и является ссылочным,здесь мы увидим ссылку этого объект,ссылку на класс с шаблоном,поскольку именно оттуда мы создали эту точку и наделили ее характеристиками

            //для наглядности: как мы уже выяснили class 'Point' - является просто шаблоном,там не создуются экземпляры класса,там просто леэат поля которые мы можем использовать
            // к примеру для создания множества экземпляров класса, для которых будут доступны все переменные(поля) - описанные в нашем шаблоне(class 'Point')

            Point point3 = new Point(); // создадим третью точку

            point3.x = 3;
            point3.y = 6;
            point3.color = "Green"; // и присвоим ей характеристику - цвет
            System.Console.WriteLine($" точка 3 содержит - {point3.color} цвет");// как мы видим данной точке присвоилась еще одна характеристика

            // мы так же можем сделать иначе,создав еще один класс со всеми допустимыми цветами


            Point point4 = new Point(); // опять же создаем точку

            point4.color1 = Color.Green; // а тут для описания цвета - мы обращаемся к шаблону 'Color' и уже оттуда берем любой цвет

            System.Console.WriteLine($"цвет точки4 - {point4.color1}");

        }
    }
}

// простая задача с использованием ООП по созданию описания студента
namespace ALL_student
{
    ...
    class StudentInfo
    {
        ...
        static void PrintStudent(Student student){
            ...
            System.Console.WriteLine(student.first_name);
            System.Console.WriteLine(student.last_name);
            System.Console.WriteLine(student.age);
            System.Console.WriteLine(student.groupe);
            System.Console.WriteLine(student.gender);
            System.Console.WriteLine(student.character);

        }
        static void PrintFriendStudent(FriendStudent friend_student){
            System.Console.WriteLine("Друг студента: ");
            System.Console.WriteLine(friend_student.reliable);
            System.Console.WriteLine(friend_student.age_friend);
            System.Console.WriteLine(friend_student.family);
            System.Console.WriteLine(friend_student.police);
        }
        static void Main(){
            Student first_student = GetStudent();
            FriendStudent first_friend_student = GetFriendStudent();
            PrintStudent(first_student);
            PrintFriendStudent(first_friend_student);
        }
    }
}
...
var lambda = (int x,int y) => x + y; //инициализация лямбда функции,присвоение параметров,тело лямбда функции выполняется после ключевого '=>'
lambda(15,30); // вызов происходит точно так же как и с обычной функцией,точно так же как и в обычной функции сюда передаются аргументы
//плюс такой функции в том что она компактна

int Factorial(int number) // посчитаем факториал с помощью такой функции
{
    int sum = 1; 
    var lambda_factorial = (int x,int sum) => x * sum; // в блоке такой функции будем умножать текущую сумму на текущий элемент
    for(int i = number;i > 0;i--) sum = lambda_factorial(i,sum); // начинаем с конца и будем каждый раз уменьшать значение,на самом деле роли это не играет,каждую итерацию вызываем лямбда функцию и присваиваем ее значение сумму(sum)
    return sum;
}


int CountUpperLetter(string basic_string)
{
    int count = 0;
    var letter_is_upper = (char letter) => char.IsUpper(letter); // лямбда функция которая будет принимать текущую букву и возвращать true либо false в зависимости от того в каком регистре буква
    foreach(char e in basic_string) if(letter_is_upper(e)) count++; // проходим каждую букву в слове и передаем ее в лямбда функцию,если она возвращает true - увеличиваем счетчик
    return count; // количество букв в верхнем регистре в слове
}


void UnicLetter(string basic_string) // нахождение буквам пар
{
    var check_unik = (char letter,char check_letter) => char.ToLower(letter) == char.ToLower(check_letter); // в блоке лямбда функции будем проверять текущую букву со всеми остальными в слове,если они равны будем создавать пару
    for(int i = 0;i < basic_string.Length;i++){ // текущая буква на проверку
        for(int j = i + 1;j < basic_string.Length;j++) // все остальные
            if(check_unik(basic_string[i],basic_string[j])) System.Console.WriteLine($"{basic_string[i]}:{basic_string[j]}"); // вызов нашей лямбда функции
    }
    return;
}

Console.Write(UnicLetter("LAMBDAA"));



                                                                                    //html & css
/*
комментарий в html ставится следующим образом <!-- описание комментария -->
каждый сайт должен содержать 'footer' - нижний колонтитул,посути подчеркивание в самом низу страницы
далее идет 'content' - основное наполнение страницы,контент на ней,к примеру на сайте по продаже одежды
в разделе 'content' - будет некая одежда(основное наполнение сайта),так же можно называть 'main'
далее 'menu' - кнопочки сверху страницы,дабы было проще ориентироваться на самом сайте,к примеру там могут быть ссылки на различные компоненты
допустим 'my profile' - такая кнопка будет переодресовывать пользователя в его профиль и т.д.
и самым последним является шапка 'header' - там может быть лого нашей компании,либо же просто красивая картинка

так же рекомендуется указывать все контактные данные либо же информацию о соденржании сайта - в разделе 'footer' - тоесть внизу страницы

так же меню может распологаться в разделе header,это определяет сам разработчик,как таковых основных правил может и не быть

ВАЖНО ЗАПОМНИТЬ,что наше окно сайта может быть как горизонтальным так и вертикальным

ОСНОВНЫЕ ВИДЫ ДИЗАЙНА:
1.    резиновый - тот дизайн,в котором ширина дизайна/рисунка задана в процентах от текущего разрешения
2.    фиксированной ширины - тот дизайн,в котором ширина столбца/рисунка задана в пикселях,тоесть оговорена точно
3.    адаптивный -  тот дизайн который,адаптивно подстраивается под размер экрана,может происходить перестройка и сдвигание блоков(к примеру блоков меню)

ВИДЫ СОДЕРЖИМОГО САЙТА:
1.    статический - содержимое которое происходит по вычислениям внутри нашего сервера,тоесть подготовлен заранее и просто выдается пользователю
      ОСОБЕННОСТИ:
        если приводить пример,то
        вот это Console.WriteLine("Hello world") - статическое содержимое,потому что чтобы вывести другую информацию вместо Hello world
        нам нужно изменить данные внутри метода Console.WriteLine вручную
        а если мы напишем int number = 10 Console.WriteLine($"данные: {number}") - то вывод на экран будет зависеть от переменной number,если меняется она то и вывод на экран меняется
2.    динамический - то содержимое которое генерируется динамически при помощи серверных языков программирования
      ОСОБЕННОСТИ:
        любое содержимое сайта которое написано на языке программирования является динамическим(html и css - не являются яп)
        тоесть если к примеру на нашем сайте мы хотим отображать курс валют,то мы явно не будем каждый раз писать код на html а потом каждый раз вручную его менять,когда курс изменится
        мы напишем код который будет обращаться к базе данных,в которой будет храниться информация о курсе валют
        и будем отображать данные оттуда динамически
...
протокол используется для запроса на сервер
грубо говоря когда пользователь заходит на наш сайт
на наш сервер запрашивается определенная страница,указаная в url адресе
если такая страница есть,сервер возвращает нам эту страницу 
тоесть для соединения нашего пользователя с сервером и импользуются протоколы

ОСНОВНЫЕ ЭТАПЫ СОЗДАНИЯ САЙТА:
1.конечно же идея,к примеру нам в голову пришла идея создать сайт игрушек для детей
2.передача этой идеи в руки дизайнера,нам ведь нужен логотип,картинки на сайте,всем этим занимается дизайнер и происходит это в первую очередь
3.далее после того как у нас есть идея,логотип,фотки товара к примеру,все это переходит к фронтенд разработчику,который уже создает видимую часть сайта
  header,footer,main,menu - как мы говорили ранее
4.наш сайт стал видим,однако функционала он пока что не имеет,именно поэтому следующий этап - это создания функционала,этим занимается backend разработчик,такое название пошло от того что наш функционал явно будет работать с database(базой данных)
  однако не весь функционал,что есть на сайте создает backend разработчик,такие вещи к примеру,как скрол страницы,листание фоток и другие примитивные вещи - создает так же frontend,на основе js к примеру
  тоесть backend это основной функционал сайта,к примеру вычисления,регистрация,авторизация.А такие вещи которые не требуют каких то вычислений,связи с bd создает так же frontend
5.и так когда наш сайт готов,последним этапом является тестировка и продвижение нашего проекта,этим занимаются как не странно тестировщики и smm менеджеры

                                                                                    //ТЕГИ
выделим для себя,что существуют как парные так и одиночные теги
ПАРНЫЕ ТЕГИ:
ПАРНЫЕ ТЕГИ ВСЕГДА ДОЛЖНЫ БЫТЬ ОТКРЫТЫ И ЗАКРЫТЫ 
вспомним нашу поездку в другую страну к примеру,когда мы проезжаем какой-то город,при въезде мы видим табличку с названием города а при выезде видим тоже самое название,но уже зачеркнутое
так же и тут:
1.тег параграфа <p>Классный параграф</p>,как мы видим тег '<p>' сначала открылся,далее туда передался контент,а далее тег закрылся,тоесть то что нужно обработать тем либо инным тегом,должно находиться между закрытием и открытием тега
2.тег жирного шрифта <b>Жирный текст</b>
3.<i>курсивный текст</i>
4.<a>Меню</a> - тег переодрисации,который создает кнопку ссылки на определнный файл,который указывается в атрибуте,в теле тега указывается название самой кнопки переодрисации,которая появится у нас на экране
  так же в этот тег можно передавать асболютные ссылки
  так же в теге 'a' можно создать якорь
  создается он так:
  к примеру где-то вверху страницы у нас есть заголовок <h1>Main</h1>
  а где-то снизу мы хотим создать прокрут страницы вверх
  для этого мы в заголовке <h1>Main</h1> указываем уникальный id <h1 id="top">Main</h1>,так же как и отребуцт указываем мы его в ковычках
  далее этот уникальный id мы просто передаем в атрибут href тега <a> - </a href="#top">вверх страницы</a>
  таким образом наша ссылка вернет нас на этот id поскольку он был указан в начале вернемся мы в начало
  по аналогии так же можно сделать и с серединой
  сделав в середине тег и дав ему уникальный id,далее просто передать этот id,как атрибут href
   
5.<head></head> - по аналогии с жизнью,голова это самый важный компонент,так же и тут все самое важное для браузера(именно для браузера),находится в теге <head>
6.<body></body> - а вот уже все что будет видно и нужно нам для работы находится в теге <body>
7.<html></html> - думаю очевидно,что данный тег указывает где у нас начинается и заканчивается html файл
8.<title></title> - тег который задает название страницы в браузере окон,пишется в теге <head>,думаю понятно,что в теле тега указывается само название
9.<ul> - данный тег нам подсказывает,что есть немаркированный список(unorderred list)
    <li>первый элемент</li> -элементы этого списка 
    <li>второй элемент</li>
    <li>третий элемент</li>
</ul>
внешний вид будет выглядеть так:
первый элемент
второй элемент
третий элемент
10.<ol> - данный тег создает уже нумерованный список(orderred list)
        <li>первый элемент</li> - здесь объекты будут уже пронумерованны 
        <li>второй элемент</li>
        <li>третий элемент</li>
...
2.второй элемент
3.третий элемент
11.<h1>Заголовок</h1> - данный тег создает заголовок первого уровня(как с книгой)
   <h2></h2> - так же есть заголовок второго уровня и так далее до 6 '<h5></h5>',чем число меньше тем заголовок больше
12.в тегах так же можно использовать p*3,далее нажать tab и тег p напишется 3 раза,в зависимости от того насколько умножили,так же можно выделить несколько строк кода,зажать 'alt' - таким образом мы сможем манипулировать сразу всеми выделенными строками кода
ОДИНОЧНЫЕ ТЕГИ:
одиночными тегами называют,как бы точечные действия,к примеру в теге <p>Я хочу: перенести строку</p> я хочу перенести строку в ее середине,но при этом хочу чтобы она была в теге параграфа 
это можно записать так <p>Я хочу:<br>перенести строку</br></p> - таким образом все что в блоке тега <br> - перенесется на другую строку
1.<br></br> - тег переноса некого текста на другую строку,сокращенно break
2.<hr></hr> - создает горизонтальную черту снизу слова


                                                                                    //АТРИБУТЫ ТЕГОВ
1.рассмотрим атребут для тега <a></a> - как мы уже помним это тег переодрисации,сокращенно ancore(якорь)
  у этого тега есть атрибут href - который как раз и переодрисует нас на другую страницу,другой страницей выступает другой файл на нашем компьютере
  и так в теге <a href="index.html">Кнопка</a> - мы указываем название файла куда будет произведена переодрисация,после чего указываем название кнопки,после чего закрываем тег                                                                                                                                                                 
ВАЖНО ЗНАТЬ:
    значение атрибута всегда должно содержать только строчные буквы,на английском а так же быть в кавычках


                                                                                    //СПЕЦСИМВОЛЫ
такие символы начинаются с '&' и заканчиваются ';' 
1.&nbsp; - неразрывный пробел(no braking space),к примеру мы хотим написать,что размер нашей комнаты 6 м на 8 м,однако тот кто будет это читать не совсем поймет о чем речь
нам нужно соединить 6 и м,для этого пишем так: <p>Размер комнаты 6&nbsp;м на 8&nbsp;м</p> - и пробел у нас становится неразрывным                                                                           


                                                                                    //ГИПЕРССЫЛКИ
ссылки бывают разных типов:
1.относительные - это ссылки которые распологаются рядом,относительно друг друга,тоесть это может быть ссылка на файл на нашем компьютере 
2.абсолютная - это ссылка из сети интернет,тоесть ссылка просто взятая из интернета
3.якорь - чаще всего это такая кнопочка,которая адресует нас куда либо,как с тегом <a href="index.html">Кнопка</a> и его атрибутом href                                                                             


                                                                                    //ФОРМЫ И ИХ ЭЛЕМЕНТЫ
создание формы,происходит точно так же как и тега <form action =""></form>,где в атрибут action передается куда мы хотим передать значения
если мы поставим '#' - то мы укажем,что данные никуда не передаются
далее в этой форме можем добавить надпись а рядом с ней ввод пароля к примеру
пишем:
<form action="#">
        Input name: <input type="text">
</form>
после чего у нас появится форма ввода имени
далее можем добавить форму ввода пароля
<form action="#">
        Input name: <input type="text">
        Input password: <input type="password">
</form> 
однако здесь элементы,что будет вводить пользователь будут иметь тип 'password',это специальный тип нужный как раз таки для ввода пароля
добавим так же имейл и дату рождения
<form action="#">
        Input name: <input type="text">
        Input password: <input type="password">
        Input email: <input type="email">
        Input age <input type="date">
</form>        
как мы видим для ввода имейла и даты рождения,тип так же будет другим,нам ведь нужно понимать что пользователь вводит не просто текст(как с именем),а какие то значения(пароль,дата,имейл)  
далее можем так же добавить в нашу форму кнопку 'отправить' и 'сбросить'
<form action="#">
    Input name: <input type="text">
    Input password: <input type="password">
    Input email: <input type="email">
    Input age <input type="date">
    <button type="submit">Отправить</button> - кнопки так же имеют тип выполняемого действия,тут он интуетивно понятен
    <button type="reset">Сбросить</button>
</form>  
добавим так же теги переноса на следующую строку
<form action="#">
    Input name: <input type="text"><br>
    Input password: <input type="password"><br>
    Input email: <input type="email"><br>
    Input age <input type="date"><br>
    <button type="submit">Отправить</button><br> - кнопки так же имеют тип выполняемого действия,тут он интуетивно понятен
